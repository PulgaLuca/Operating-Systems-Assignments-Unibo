...
{ 
	...
	*/
	...
	while(1) {
		...
		while(( ponteoccupato ) || ( myBiglietto>biglietto[indiceSenso] ) || ( numInAttesa[indiceSenso]<numInAttesa[!indiceSenso] ) ||
			(numInAttesa[indiceSenso]==numInAttesa[!indiceSenso] && indiceSenso==INDICESENSOANTIORARIO)){
			DBGpthread_cond_wait(&condPonteLibero, &mutex, NULL);
		}
		...
		DBGpthread_mutex_lock(&mutex,Plabel);
		ponteoccupato=0;
		DBGpthread_cond_broadcast(&condPonteLibero, &mutex);
		DBGpthread_mutex_unlock(&mutex,Plabel);
